...
// Normalisasi Role ke lowercase agar aman
const userRole = computed(() => (user.role || '').toLowerCase());
const isOperatorOrAdmin = computed(() => ['operator', 'admin', 'approval'].includes(userRole.value));
...
// Inisialisasi Chart
onMounted(() => {
    if (isOperatorOrAdmin.value && props.chart_data) {
            ...
            new Chart(ctx, {
                ...
                data: {
                    ...
                    datasets: [
                        {
                            label: 'Barang Masuk',
                            data: props.chart_data.masuk || [], 
                            backgroundColor: '#0d9488', // teal-600
                            borderRadius: 4
                        },
                        {
                            label: 'Barang Keluar',
                            data: props.chart_data.keluar || [],
        ...
        <div class="py-8 px-4 md:px-6 lg:px-8">
            ...
            <div class="mb-6">
                ...
                <p class="mt-1 text-sm text-gray-600">
                    Selamat datang kembali, <span class="font-semibold text-teal-700">{{ user.name }}</span>!
                </p>
            </div>

            <div v-if="isOperatorOrAdmin" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <h3 class="text-2xl font-bold text-slate-800">{{ props.stats_operator?.total_barang || 0 }}</h3>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <p class="text-sm text-gray-500 font-medium">Permintaan Baru</p>
                                <h3 class="text-2xl font-bold text-slate-800">{{ props.stats_operator?.permintaan_pending || 0 }}</h3>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <p class="text-sm text-gray-500 font-medium">Keluar (Bulan Ini)</p>
                                <h3 class="text-2xl font-bold text-slate-800">{{ props.stats_operator?.barang_keluar || 0 }}</h3>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <p class="text-sm text-gray-500 font-medium">Stok Menipis (≤5)</p>
                                <h3 class="text-2xl font-bold text-slate-800">{{ props.stats_operator?.stok_menipis || 0 }}</h3>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div class="lg:col-span-2 bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                        <div class="flex justify-between items-center mb-4">
                            <h3 class="text-lg font-bold text-slate-800">Statistik Mutasi Barang ({{ new Date().getFullYear() }})</h3>
                        </div>
                    ...
                    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                        ...
                        <div class="space-y-4 max-h-80 overflow-y-auto pr-2 custom-scrollbar">
                            <div v-for="(log, idx) in props.aktivitas" :key="idx" class="flex gap-4 p-3 bg-gray-50 rounded-lg items-start hover:bg-gray-100 transition-colors">
                                <div 
                            ...
                            <div v-if="!props.aktivitas || props.aktivitas.length === 0" class="text-center text-gray-500 text-sm py-4">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm text-left text-gray-500">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                <tr v-if="!props.permintaan_terbaru || props.permintaan_terbaru.length === 0">
                                    <td colspan="5" class="px-6 py-8 text-center text-gray-500 italic">Tidak ada permintaan baru yang perlu diproses.</td>
                                </tr>
                                <tr v-for="item in props.permintaan_terbaru" :key="item.id" class="bg-white hover:bg-gray-50 transition-colors">
                                    <td class="px-6 py-4 font-medium text-gray-900">
                                        {{ item.pemohon?.name || 'Unknown' }}
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        <Link :href="route('permintaan.index', item.id)" class="inline-flex items-center text-white bg-teal-600 hover:bg-teal-700 font-medium rounded-md text-xs px-3 py-1.5 transition-colors shadow-sm">
                                            Proses
                                        </Link>
                                    </td>
                                </tr>
            ...
            <div v-else class="space-y-8">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                    <div class="md:col-span-1 bg-gradient-to-br from-teal-600 to-teal-800 rounded-2xl p-6 text-white shadow-lg flex flex-col justify-between relative overflow-hidden">
                        ...
                        <div class="relative z-10">
                            ...
                            <p class="text-teal-200 text-xs mt-1 capitalize">{{ user.unit_kerja?.nama_unit || user.role }}</p>
                        </div>
                    ...
                    <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex flex-col justify-center items-center hover:shadow-md transition-shadow">
                        <span class="text-4xl font-extrabold text-gray-800">{{ props.stats_pemohon?.total || 0 }}</span>
                        <span class="text-sm text-gray-500 mt-2 font-medium">Total Pengajuan</span>
                    </div>
                    <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex flex-col justify-center items-center hover:shadow-md transition-shadow">
                        <span class="text-4xl font-extrabold text-yellow-600">{{ props.stats_pemohon?.proses || 0 }}</span>
                        <span class="text-sm text-gray-500 mt-2 font-medium">Sedang Diproses</span>
                    </div>
                    <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex flex-col justify-center items-center hover:shadow-md transition-shadow">
                        <span class="text-4xl font-extrabold text-green-600">{{ props.stats_pemohon?.disetujui || 0 }}</span>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
                        <div v-for="barang in props.barangs" :key="barang.id" class="group bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 p-4 flex flex-col items-center text-center cursor-pointer">
                            ...
                            <div class="mt-3 flex items-center gap-2">
                                ...
                                <span class="text-[10px] font-bold px-2 py-0.5 rounded-full bg-green-100 text-green-700 border border-green-200">
                                    Ready
                                </span>
                            </div>
                        </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm text-left text-gray-500">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                <tr v-if="!props.riwayats || props.riwayats.length === 0">
                                    <td colspan="5" class="px-6 py-8 text-center text-gray-500">Belum ada riwayat pengajuan.</td>
                                </tr>
                                <tr v-for="item in props.riwayats" :key="item.id" class="bg-white hover:bg-gray-50 transition-colors">
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <Link :href="route('permintaan.index', item.id)" class="text-blue-600 hover:text-blue-800 hover:underline font-medium text-xs">
                                            Detail
                                        </Link>
                                    </td>
                                </tr>
