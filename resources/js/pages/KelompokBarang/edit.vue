...
const props = defineProps({ kelompok: Object });

const form = useForm({
  kode_kelompok: props.kelompok.kode_kelompok,
  nama_kelompok: props.kelompok.nama_kelompok,
});

const submit = () => {
  form.put(route('kelompok-barang.update', props.kelompok.id));
};
</script>
  ...
  <AuthenticatedLayout>
    <div class="py-12 max-w-2xl mx-auto px-4">
      <div class="bg-white p-6 rounded-lg shadow-sm border">
        <h2 class="text-xl font-bold mb-4">Edit Kelompok Barang</h2>
        
        <form @submit.prevent="submit">
          <div class="space-y-4">
            <div>
              <label class="block text-sm font-medium text-gray-700">Kode Kelompok</label>
              <input v-model="form.kode_kelompok" type="text" class="mt-1 w-full border-gray-300 rounded-md focus:ring-teal-500 uppercase">
            ...
            <div>
              ...
              <input v-model="form.nama_kelompok" type="text" class="mt-1 w-full border-gray-300 rounded-md focus:ring-teal-500">
          ...
          <div class="mt-6 flex justify-end gap-3">
            ...
            <button type="submit" :disabled="form.processing" class="px-4 py-2 bg-teal-600 text-white rounded hover:bg-teal-700">Update</button>
          </div>
        </form>
      </div>
    </div>
