...
import { FileText, Printer, Search, AlertTriangle } from "lucide-vue-next";
...
const applyFilter = () => {
    selectedIds.value = [];
    router.get(route('laporan.stock-opname'), filterForm.value, {
        preserveState: true,
        preserveScroll: true,
    });
};
...
const exportData = (type) => {
    ...
    const url = route('laporan.stock-opname.export', params);
    ...
    <AuthenticatedLayout>
        <div class="py-8 px-4 md:px-6 lg:px-8">
            ...
            <div class="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900">Laporan Stock Opname</h1>
                    <p class="mt-1 text-sm text-gray-600">Rekapitulasi hasil audit stok fisik gudang.</p>
                </div>
                ...
                <div class="p-5 border-b border-gray-100 bg-gray-50/50">
                    <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                        <div class="md:col-span-3">
                            <label class="block text-xs font-medium text-gray-500 mb-1">Cari No Opname</label>
                            <div class="relative">
                                <Search class="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" />
                                <input type="text" v-model="filterForm.search" placeholder="Contoh: SO-2025..." 
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Ada Selisih</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <tr v-for="item in laporan.data" :key="item.id" 
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-mono font-bold text-teal-700">{{ item.no_opname }}</td>
                                ...
                                <td class="px-6 py-4 text-center">
                                    <span v-if="item.item_selisih > 0" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">
                                        <AlertTriangle class="w-3 h-3 mr-1" />
                                        {{ item.item_selisih }} Item
                                    </span>
                                ...
                                <td class="px-6 py-4 text-center">
                                    <span class="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full"
                                        :class="item.status === 'Selesai' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800'">
                                        {{ item.status }}
                                    </span>
                                </td>
                            </tr>
