        ...
        <div class="py-12">
            <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
                ...
                <div class="flex justify-end mt-6 gap-x-3 px-4 sm:px-0">
                    ...
                    <!-- Tambah Kategori: HANYA OPERATOR -->
                    <Link v-if="isOperator" :href="route('kategori.create')" class="flex items-center px-4 py-2 text-white bg-teal-600 rounded-md hover:bg-teal-700">
                ...
                <div class="mt-4 overflow-hidden bg-white rounded-lg shadow-sm ring-1 ring-gray-900/5 mx-4 sm:mx-0">
                    <div class="flex flex-col items-center justify-between p-4 border-b border-gray-200 md:flex-row">
                        ...
                        <div class="relative w-full mt-4 md:w-1/3 md:mt-0">
                            ...
                            <input type="text" v-model="search" placeholder="Cari kode atau nama kategori..." class="w-full py-2 pl-10 border border-gray-300 rounded-md shadow-sm focus:ring-teal-500 focus:border-teal-500" />
                        </div>
                    </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <tr v-if="kategoris.data.length === 0">
                                    <td :colspan="isOperator ? 5 : 4" class="px-6 py-4 text-center text-gray-500">Tidak ada data kategori.</td>
                                </tr>
                                <tr v-for="(kategori, index) in kategoris.data" :key="kategori.id" class="hover:bg-gray-50">
                                    <td class="px-6 py-4 text-sm text-center text-gray-500">{{ (kategoris.current_page - 1) * kategoris.per_page + index + 1 }}</td>
                    ...
                    <div v-if="kategoris.data.length > 0" class="flex items-center justify-between p-4 border-t border-gray-200">
                        <div class="text-sm text-gray-700">Menampilkan <span class="font-medium">{{ kategoris.from ?? 0 }}</span> sampai <span class="font-medium">{{ kategoris.to ?? 0 }}</span> dari <span class="font-medium">{{ kategoris.total }}</span> hasil</div>
                        <div class="flex gap-1">
                            <template v-for="(link, k) in kategoris.links" :key="k">
                                <Link v-if="link.url" :href="link.url" v-html="link.label" class="px-3 py-1 text-sm border rounded-md bg-white text-gray-700 border-gray-300 hover:bg-gray-50" :class="{ 'bg-teal-600 text-white border-teal-600 hover:bg-teal-600': link.active }" preserve-scroll />
